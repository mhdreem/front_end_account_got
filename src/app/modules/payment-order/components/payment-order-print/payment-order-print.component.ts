...
export class PaymentOrderPrintComponent {
  ...
  ngOnChanges(){
    this.Load_Data();
    this.data.payment_order_details?.sort((a, b)=> a.debtor! - b.debtor! );
    this.data.payment_order_details?.forEach(detail =>{
      this.debtor_sum+= detail.debtor!;
      this.creditor_sum+= detail.creditor!;
    });
    console.log('this.data.sanad_kid_details', this.data.payment_order_details);
  }
