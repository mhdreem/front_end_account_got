...
export class SanadKidDetailComponent implements OnChanges ,OnDestroy,OnInit {
  ...
  onAccountFill(source: string){
    if (source == 'name'){
      this.account_id.setValue(+this.accounts_tree_list.find(account=>account.seq == this.accounts_tree_fk.value)?.account_id!);
    }
    else if (source == 'id'){
      this.accounts_tree_fk.setValue(this.accounts_tree_list.find(account=>+account.account_id! == this.account_id.value)?.seq!);
    }
  }
  ...
  onAccountNameFilling(){
    ...
    console.log('details', this.sanad_kid_detail);
  }
